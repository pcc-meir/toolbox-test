...
const JwtDecoder = () => {
  ...
  const exampleJwt = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyLCJleHAiOjE1MTYyMzkwMjJ9.4Adcj3UFYzPUVaVF43FmMab6RlaQD8A9V8wFzzht-KQ";
  const exampleJSON = {
    header: {
      alg: "HS256",
      typ: "JWT"
  ...
  useEffect(() => {
    if (mode === "Decode") {
      ...
      try {
        const jwtPayload = jwtDecode(value);
        setOutput(`Issued date:
${jwtPayload.iat && new Date(jwtPayload.iat * 1e3)}

Expiration date:
${jwtPayload.exp && new Date(jwtPayload.exp * 1e3)}
...
${JSON.stringify(jwtPayload, null, 2)}`);
      ...
      try {
        ...
        if (keyExists(inputJSON) && payloadExists(inputJSON) && headerExists(inputJSON)) {
          ...
          (async () => {
            const token = await new SignJWT(inputJSON.payload).setProtectedHeader(inputJSON.header).setIssuedAt(inputJSON.payload.iat).setIssuer(inputJSON.payload.iss).setExpirationTime(inputJSON.payload.exp).sign(secret);
            setOutput(token);
          })();
        }
...
export { JwtDecoder, JwtDecoder as default };
//# sourceMappingURL=JwtDecoder.esm.js.map
